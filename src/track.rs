...
#[cfg(test)]
mod a_ledger_scope_over_an_untracked_stage {
    //! **Relocated from `highbay_data`'s `tests/assemble_under_the_driver.rs`**
    //! at the visibility flip, where it stood as
    //! `the_ledger_scope_changes_speed_and_not_answers` over the REAL assembly
    //! stage.
    //!
    //! It cannot live there any more: it composes `Tracked::new(&ledger, label,
    //! Memo::new(stage, store))` by hand - the order [`Memo`]'s own doc requires,
    //! tracking OUTSIDE and memo inside - and the builder has no spelling for a
    //! tracked graph (`DESIGN.md`'s finding 1), so with the flat exports gone
    //! [`Ledger`] and [`Tracked`] are not nameable from a consumer at all.
    //!
    //! What is held here is the property, over a stand-in. What is PARKED is
    //! holding it over a real stage, which is the half only a consumer can
    //! supply; `assemble_under_the_driver.rs` carries a note saying so and the
    //! four assertions come back the day the builder grows `.tracked_stage(..)`.
    //!
    //! The case is deliberately a stage that reads NO tracked state - assembly
    //! reads none - so the ledger has nothing to invalidate and the whole of
    //! what is measured is that adding the scope changes SPEED and not ANSWERS.
    //! [`the_memo_over_tracked_state_changes_speed_and_not_answers`] is the same
    //! control over a stage that does read tracked state; this is its twin on
    //! the other side of that line.
    //!
    //! [`the_memo_over_tracked_state_changes_speed_and_not_answers`]:
    //!     super::invalidation_marks_dependents
    ...
    use crate::driver::{FrameDriver, NoPendingWork, run_to_completion};
    use crate::memo::Memo;
    use crate::track::{Ledger, Tracked};

    /// Stand-in for whatever an author wrote.
    #[derive(Clone, PartialEq, Eq, Debug)]
    struct Source(&'static str);

    /// A pure, keyed stage that reads nothing ambient, with a count of the polls
    ...
    impl Stage for Pure {
        ...
        fn poll_stage(
            ...
            *self.runs.lock().unwrap() += 1;
            EffectPoll::Ready(input.0.to_uppercase())
        }
    }

    fn offline<S>(stage: &S, input: &Source) -> String
    where
        S: Stage<Input = Source, Output = String, Error = &'static str>,
    {
        run_to_completion(stage, input, &NoPendingWork).expect("a pure stage answers")
    }

    fn per_frame<S>(stage: &S, input: &Source) -> String
    where
        S: Stage<Input = Source, Output = String, Error = &'static str>,
    {
        let driver = FrameDriver::new();
        let polled = driver.poll_frame(stage, input);
        assert!(
            !driver.take_stale(),
            "the stage answered without waiting, so nothing should have asked for a redraw",
        );
        polled.ready().expect("a pure stage answers on the first poll")
    }

    #[test]
    fn the_ledger_scope_changes_speed_and_not_answers() {
        ...
        assert_eq!(
            tracked.stage().stage().runs(),
            1,
            "the lookup sits inside the node's scope, so an unstale node hits",
        );
        assert_eq!(first, second, "the ledger must not change what a stage answers");
        assert_eq!(
            first,
            offline(&Pure::new(), &input),
            "nor differ from the bare stage's answer",
        );
        assert!(
            !ledger.is_stale(tracked.node()),
            "a node that produced a value is not stale",
        );
    }
}
